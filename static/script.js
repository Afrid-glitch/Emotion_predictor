(() => {
  ...
  const GAUGE_ARC_LENGTH = 282.7; // length of the semicircular path, r=90
  ...
  function renderResult(data, originalText) {
    ...
    el.confidenceText.textContent = `${(confidence * 100).toFixed(1)}% confidence`;
  ...
  function renderGauge(confidence) {
    ...
    el.gaugeFill.style.strokeDashoffset = String(GAUGE_ARC_LENGTH * (1 - confidence));
  }
